-- Bloquea admins/superadmins en hacer_puja (solo lectura para ellos)

CREATE OR REPLACE FUNCTION public.hacer_puja(
  p_remate_id uuid,
  p_horse_id uuid,
  p_monto numeric,
...
begin
  ...
  if coalesce(v_is_admin, false) or coalesce(v_is_super, false) then
    raise exception 'Admins no pueden pujar';
  end if;
  ...
  if v_remate.estado <> 'abierto' then raise exception 'El remate no esta abierto'; end if;
  ...
  if coalesce(p_es_manual, false) = false then
    ...
    if p_monto is null or p_monto < (v_minimo_permitido + 10) then
      raise exception 'Oferta manual minima: % (auto: %)', (v_minimo_permitido + 10), v_minimo_permitido;
    end if;
  end if;

  -- 7) Calcula cuanto bloquear realmente
  ...
  -- 8) Lock wallet del que esta pujando
  select *
    into v_wallet
  from public.wallets
